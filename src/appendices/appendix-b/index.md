...
- 利用可能な資産と対応commit: [付録A：Companion資産カタログ](../appendix-a/)

この付録の導線は、付録Aで `shipped` と確認した資産だけを使用します。
専用assetが未提供の領域は、汎用templateとpolicyを組み合わせ、存在しない固定pathを前提にしません。
...
GitHub AgentOps では、Copilot、GitHub Actions、MCP、OpenAI Codex などの公式情報が短期間で変わります。
リンク切れの有無だけでなく、本文が参照している plan、preview 状態、quota、課金、権限、API 名称が
現在の公式情報と矛盾していないかを確認します。

推奨手順：

1. 対象範囲を決める
   - 全文棚卸し、特定章、リリース前確認、四半期レビューのどれかを Issue に明記する
   - 料金、preview、quota、Actions minutes、security boundary など、変化しやすい項目を優先する
2. 外部リンク検査を実行する
   - 基本コマンドは `npm run check-external-links`
   - DNS や社内 proxy の制限で到達できない host は、恒久対応ではなく一時的な検査除外として記録する
3. 一次情報と本文の対応を確認する
   - GitHub Docs、OpenAI Developers、公式 repository など、一次情報を優先する
   - blog、community 記事、個人記事を根拠にする場合は、本文の主根拠にしない
4. source と生成物を同期する
   - 原稿は `src/` を編集し、`npm run build` で `docs/` を更新する
   - top page や README の導線を変更した場合は、公開 smoke の確認対象に含める
5. PR evidence を残す
   - 実行日、検査コマンド、確認した代表リンク、未対応リンク、再確認が必要な項目を PR 本文に記録する

停止条件：

- 公式情報の現行名称や提供状態が確認できない
- 料金、quota、preview 状態など、誤ると読者の意思決定に影響する情報の根拠が取れない
- 一時的なネットワーク障害と恒久的なリンク切れを切り分けられない

完了 evidence：

- `npm run check-external-links` の結果
- 参照元章と代表リンクの確認日
- `npm test` と公開 Pages smoke の結果
- 次回棚卸しで再確認すべき変化しやすい項目
...
- Review prompt: `.github/codex/prompts/pr-review.md`
...
- Issue テンプレ: `.github/ISSUE_TEMPLATE/agent-task.yml`
- Policy: `rules/command-policy.md`

security専用Issue form / SkillはCompanionでは未提供です。汎用templateへthreat、secret、permission、rollbackの受入基準を追加し、
専用assetが必要ならCompanion側の独立Issueで設計します。
...
- Issue テンプレ: `.github/ISSUE_TEMPLATE/agent-task.yml`
- Scope例: `custom-agents/MCP_SCOPE_EXAMPLE.md`

MCP専用Issue form / review noteはCompanionでは未提供です。上記scope例を完成済みreview templateと誤認せず、
導入先のtool、権限、side effect、失効条件をIssueへ具体化します。
...
- 共通Policy: `rules/command-policy.md`

Actions permission専用noteはCompanionでは未提供です。共通command policyに加え、GitHub公式仕様と対象workflow差分を正本にします。
...
既存 Companion では `.github/workflows/codex-pr-review.yml` / `.github/workflows/codex-release-prep.yml` を採用しています。
...
- Issue テンプレ: `.github/ISSUE_TEMPLATE/improvement.yml`

rollout専用Issue formとscorecardはCompanionでは未提供です。指標定義を汎用改善Issueへ記録し、個人評価へ転用しません。
