---
layout: book
title: "付録A：テンプレ集（instructions / Issue / PR / agents / Skills / hooks / MCP）"
---
...
| 資産 | 固定パス候補 | 推奨する扱い |
| --- | --- | --- |
| PR summary / risk extraction | `.github/workflows/codex-pr-review.yml` | 既存固定パス。comment-only から開始し、必須 gate にしない |
| Copilot review checklist | `.github/review-templates/copilot-review.md` | suggestion 対応、却下理由、再レビュー条件を記録 |
| Release prep | `.github/workflows/codex-release-prep.yml` | 既存固定パス。`workflow_dispatch` と対象 ref を明示 |
...
既存 Companion では `codex-pr-review.yml` と `codex-release-prep.yml` を固定パスとして扱います。
`agentops-*` へ改名する場合は、README と本書の参照を同じ PR で更新し、互換期間を置いてから旧パスを廃止します。
