...
| Copilot cloud agent | Copilot cloud agent premium request SKU、agent session、関連 workflow の実行条件 | Issue から実装依頼、PR 修正依頼 | repository owner / Copilot 管理者 |
...
ここでの Copilot usage governance と GitHub Actions cost guardrails は別の control surface です。
前者は premium request paid usage policy、premium request budgets、premium request analytics を中心に見ます。
後者は hosted runner minutes / storage の budgets、runner 選択、artifact / cache retention を中心に見ます。
Copilot cloud agent を使う運用でも、同じ依頼フローの中で両方が関係し得るため、別々に確認します。
...
月次レビューでは、最低限次を確認します。
なお、premium request analytics の粒度や閲覧権限は role により異なるため、user 単位が見えない場合は usage report や内部運用ログで補います。
