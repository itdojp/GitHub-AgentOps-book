...
エージェントが作成・更新した PR は、通常の PR よりも「完了したように見える」速度が速いため、
merge 前に同じゲートで止める必要があります。次を PR body または監査コメントに残します。

| ゲート | 記録する証跡 | 判断基準 |
| --- | --- | --- |
| 仕様 | 対応 Issue、受入基準、非スコープ | Issue から PR の変更意図を追跡できる |
| review | review 本文、inline comment、suggestion への対応 | 未回答の指摘がない |
| thread | unresolved review thread 数 | merge 前に 0 件である |
| CI | 必須 checks、失敗時の再実行理由 | green であり、skip / retry の理由が説明できる |
| 権限 | Secrets、write 権限、外部 API 利用の有無 | 承認境界と実行イベントが一致している |
| 反映 | merge 後 main checks、公開サイト、ロールバック手順 | merge 後の状態を確認できる |

特に Copilot coding agent などが Issue から PR を作成する運用では、依頼者の承認が
required approvals に数えられない場合があります。承認者、最終 push、required checks の関係を
ブランチ保護/Rulesets と合わせて確認し、人間の責任境界を曖昧にしないことが重要です。

公式情報の確認先（2026-05-23 Asia/Tokyo 時点）:

- GitHub Docs: Reviewing a pull request created by GitHub Copilot
  <https://docs.github.com/en/copilot/using-github-copilot/coding-agent/reviewing-a-pull-request-created-by-copilot>
...
- GitHub ワークフロー実践ガイド: 第12章
  <https://itdojp.github.io/github-workflow-book/chapters/chapter12/>
...
- [ ] 仕様差分が「決定」として追跡できる
- [ ] review 本文・inline comment・suggestion への対応/不要理由が記録されている
- [ ] 未解決 review thread が 0 件で、CI green と merge 後確認が記録されている
